// $Id: type_checker.h,v 1.18 2016/05/19 17:38:16 ist178190 Exp $ -*- c++ -*-
...
namespace zu {
  ...
  public:
    bool isLeftValue(cdk::basic_node * const node) {
      std::string name = typeid(*node).name();
      if (name.compare("index_node") == 0 || name.compare("variable_node") == 0)
        return true;
      return false;
    }
